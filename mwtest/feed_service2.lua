...
lkserver:setpeername('127.0.0.1', '5671')

local lkclient = socket.udp()
lkclient:setsockname('127.0.0.1', '5681')
...
function getTopView()
  ...
  for _, v in ipairs(raw_topview) do
    ...
    if not art_name:match(':') and not list_match(spamlist, art_name) then
      ...
      table.insert(topview_data.new_list, { article = art_name, views = v.views })
...
while true do
  ...
  if recvbuff then
    ...
    if topview_data.last_date ~= curdate.day and curdate.hour >= 11 and curdate.min >= 0 then
...
print('The length is ' .. #topview_data.new_list)
print(json.encode(topview_data.new_list))
